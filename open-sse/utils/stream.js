...
export function createSSEStream(options = {}) {
  ...
  // The Responses API has no [DONE] sentinel. Codex closes the socket the moment it
  // sees the terminal event, which cancels the reader -- so flush() never runs and the
  // request bills nothing. Record usage at the terminal event too, once-guarded so the
  // flush path is a no-op whenever the client did stay to the end.
  let finalized = false;
  const finalizeStream = () => {
    ...
    if (!hasValidUsage(finalUsage) && totalContentLength > 0) {
      ...
      else if (state) state.usage = finalUsage;
    }
  ...
  return new TransformStream({
    transform(chunk, controller) {
      ...
      for (const line of lines) {
        ...
        if (keepsOpenAIResponsesFormat && openAIResponsesEventName) {
          ...
          sseEmittedCount++;
          // Usage is on the terminal event and has just been extracted above. Bill here,
          // after the client has it: codex closes the socket on this chunk, which cancels
          // the reader and means flush() never gets to run.
    ...
    flush(controller) {
      ...
      try {
        ...
        if (buffer.trim()) {
          // Same parse as the transform loop: without targetFormat this only accepts
          // "data: " lines, so an NDJSON provider (Ollama) lost whatever arrived
          // without its closing newline.
          const parsed = parseSSELine(buffer.trim(), targetFormat);
          // parseSSELine turns the SSE sentinel "data: [DONE]" into { done: true },
          // which must not be translated. An Ollama chunk also carries done:true, but
          // it is the real final chunk -- it holds done_reason and the token counts --
          // so it has to go through.
          const isDoneSentinel = parsed?.done && targetFormat !== FORMATS.OLLAMA;
          if (parsed && !isDoneSentinel) {
            // Same accumulation the transform loop does, so finalizeStream() logs the
            // tail chunk's tokens instead of falling back to null.
            const extracted = extractUsage(parsed);
            if (extracted && state) state.usage = extracted;
