...
/**
 * Derive a stable RFC-4122 UUID from an arbitrary string.
 *
 * The real CLI sends plain UUIDs for x-grok-session-id / x-grok-conv-id, and
 * this fork's deriveSessionId() emits `uuid + Date.now()` (a different
 * provider's binary format). Since these headers exist to match the CLI's
 * fingerprint, we hash to a well-formed UUID instead. Hashing also keeps the
 * id stable across process restarts, which an in-memory map cannot do.
 */
function stableUuidFrom(seed) {
  const h = crypto.createHash("sha256").update(String(seed)).digest("hex");
  // Stamp version 4 and the RFC-4122 variant so it is a valid UUID.
...
/**
 * Resolve the x-grok-session-id / x-grok-conv-id the CLI sends.
 *
 * Upstream calls its own resolveSessionId(), which this fork does not have, so
 * we walk the same precedence by hand: an explicit conversation id from the
 * client wins (it is the only thing that actually tracks a thread), then the
 * workspace, then a stable id derived from the connection. The header only has
 * to be stable per conversation; clients that send no thread metadata simply
 * share one session per connection.
 */
export function resolveGrokCliSessionId(credentials, body) {
  ...
  if (typeof explicit === "string" && explicit.trim()) return stableUuidFrom(explicit.trim());

  const workspaceId = credentials?.providerSpecificData?.workspaceId;
  if (typeof workspaceId === "string" && workspaceId.trim()) return stableUuidFrom(workspaceId.trim());

  const connectionId = credentials?.connectionId || credentials?.id;
  // No connection id at all — fall back to the shared store rather than
  // minting a fresh id per request, which would break multi-turn entirely.
  if (!connectionId) return stableUuidFrom(deriveSessionId(undefined));
  return stableUuidFrom(`grok-cli:${connectionId}`);
}
