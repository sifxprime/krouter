...
export class KiroExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials, stream = true) {
    const headers = {
      ...
      // Fork divergence (0.5.73) — MITM anti-loop header. BaseExecutor.execute()
      // also force-sets this on every outbound request (0.5.74), but we keep it
      // here so a direct buildHeaders() call still carries it: Kiro is a
      // MITM-intercepted host, and without it an outbound AWS call would loop
      // back into our own /v1/messages instead of reaching CodeWhisperer.
