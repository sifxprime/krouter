...
// 0.5.111 (upstream d6761c6f) — video endpoints per provider.
//
// Upstream reads this from PROVIDER_MEDIA[provider].videoConfig in its provider
// registry, which this fork does not have. Rather than port the whole registry
// for one field, the video base URLs live here. Only xAI Grok Imagine exists
// today; add a provider by adding a { baseUrl } entry.
const VIDEO_CONFIG = {
  // Async video jobs: POST returns { request_id }, GET /{id} polls to done/failed.
  // Docs: https://docs.x.ai/developers/rest-api-reference/inference/videos
  xai: { baseUrl: "https://api.x.ai/v1/videos" },
};
...
export function getVideoConfig(provider) {
  return VIDEO_CONFIG[provider] || null;
}
