...
// This fork has no provider registry; the equivalents live in the UI manifest
// (AI_PROVIDERS) and the model catalog (PROVIDER_MODELS keyed by alias).
...
describe("video model listing (0.5.111 wiring)", () => {
  ...
  it("the main /v1/models keeps video out of the LLM list", async () => {
    const { readFileSync } = await import("node:fs");
    const src = readFileSync("src/app/api/v1/models/route.js", "utf8");
    // video is a media kind; the default catalog is LLM-only.
