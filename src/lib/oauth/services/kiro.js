...
export class KiroService {
  ...
  /**
   * 0.5.118 (upstream 706e6513) — List available CodeWhisperer profiles for a
   * token (or API key) and return the best-matching profileArn. AWS SSO OIDC
   * logins return no profileArn, so it must be fetched separately — the same
   * call works for API-key auth. Accepts both `arn` and `profileArn` response
   * field names (the API-key JSON-1.0 surface returns `arn`).
   */
  ...
  /**
   * 0.5.118 (upstream 706e6513) — Validate an API-key (ksk_) credential by
   * listing profiles with it. API keys are long-lived bearer tokens (no
   * refresh), so the only way to validate one is an authenticated
   * CodeWhisperer call. Returns a credential object ready to persist as a
   * "kiro" connection with authMethod="api_key".
   */
