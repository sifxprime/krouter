...
export async function handleChat(request, clientRawRequest = null) {
  ...
  // 0.5.125 (upstream 6498b312) — modalities the request actually carries, so the
  // capacity adapter can float in a capable fallback model when the target can't.
  ...
  if (comboModels) {
    ...
    return handleComboChat({
      ...
      handleSingleModel: withCapacityAdapterStripping(
        (b, m) => handleSingleModelChat(b, m, clientRawRequest, request, apiKey, settings, 1),
        adapterAdded
      ),
      log,
      comboName: modelStr,
...
async function handleSingleModelChat(body, modelStr, clientRawRequest = null, request = null, apiKey = null, settings = null, depth = 0) {
  ...
  while (true) {
    ...
    const result = await handleChatCore({
      ...
      headroomUrl: settings.headroomUrl || "http://localhost:8787",
